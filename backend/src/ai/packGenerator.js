...
async function generateWorthChunked({ segments, context = {} }) {
  const allBlocks = chunkSegments(segments);
  // R63 快照采样(超长音频提速, 不降质): 快照只需展示 8-12 段"最值得听",
  //   无需对全部块都跑 GLM。块数 > 上限时沿时间轴均匀采样(覆盖开头/中间/结尾, 不漏关键段),
  //   块数 ≤ 上限时全跑(短/中音频现状不变)。260min≈50块 → 采 14 块, worth 阶段 ~425s → ~120s。
  const WORTH_MAX_CHUNKS = Math.max(6, parseInt(process.env.SNAPSHOT_WORTH_MAX_CHUNKS || '10', 10));
  let blocks = allBlocks;
  if (allBlocks.length > WORTH_MAX_CHUNKS) {
    const step = allBlocks.length / WORTH_MAX_CHUNKS;
    const picked = [];
    const seen = new Set();
    for (let i = 0; i < WORTH_MAX_CHUNKS; i++) {
      const idx = Math.min(allBlocks.length - 1, Math.round(i * step));
      if (!seen.has(idx)) { seen.add(idx); picked.push(allBlocks[idx]); }
    }
    blocks = picked;
    console.log(`[worthChunked] 采样: ${allBlocks.length} 块 → 均匀取 ${blocks.length} 块(SNAPSHOT_WORTH_MAX_CHUNKS=${WORTH_MAX_CHUNKS})`);
  }
...
async function generateCardsChunked({ segments, context = {}, maxCards = 18, onProgress = null }) {
  const allBlocks = chunkSegments(segments);
  // R63 卡片采样(超长音频提速, 不降质): 18 张卡无需 50 块原料。块数 > 上限时沿时间轴均匀采样,
  //   块数 ≤ 上限全跑。260min≈50块 → 采 16 块, 卡片阶段 ~425s → ~135s。
  const CARDS_MAX_CHUNKS = Math.max(6, parseInt(process.env.CARDS_MAX_CHUNKS || '12', 10));
  let blocks = allBlocks;
  if (allBlocks.length > CARDS_MAX_CHUNKS) {
    const step = allBlocks.length / CARDS_MAX_CHUNKS;
    const picked = [];
    const seen = new Set();
    for (let i = 0; i < CARDS_MAX_CHUNKS; i++) {
      const idx = Math.min(allBlocks.length - 1, Math.round(i * step));
      if (!seen.has(idx)) { seen.add(idx); picked.push(allBlocks[idx]); }
    }
    blocks = picked;
    console.log(`[cardsChunked] 采样: ${allBlocks.length} 块 → 均匀取 ${blocks.length} 块(CARDS_MAX_CHUNKS=${CARDS_MAX_CHUNKS})`);
  }
  const genBlock = async (block, idx) => {
    const userPrompt = `转录片段 (${Math.floor(block.start / 60)}分-${Math.floor(block.end / 60)}分):\n\n`
      + block.segs.map(s => `[${Math.floor(s.start)}s] ${s.text}`).join('\n');

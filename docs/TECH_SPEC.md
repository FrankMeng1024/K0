...
**AUTH_ENABLED 生产守卫**（Sprint 2 修复）：
- `AUTH_ENABLED` 默认 `false`（开发模式，默认 user_id=1）
- `NODE_ENV=production` 时：若 `AUTH_ENABLED` 不是 `'true'`，后端启动时强制设为 `true` 并打印 `[SECURITY]` 警告
- 生产 `.env.production` 必须显式设 `AUTH_ENABLED=true` 避免每次告警

**Rate Limiting**（Sprint 2 启用）：
- 全局（IP）：60 req/min（`express-rate-limit`，windowMs=60000, max=60）
- `POST /api/episodes/import`：10 req/min/user_id
- `POST /api/episodes/:id/snapshot`：5 req/hour/user_id
- 超限返回 429 + `{ error: { code: 'RATE_LIMITED', message: '...' } }`

...
- `[pending]` **GLM 429 无重试 + 转录缓存 hit 导致秒挂**：Sprint 10 v14 后期用户反复贴同一 URL（packId=5），transcripts 缓存 hit → BCUT 45s 跳过 → 立即 GLM 生成学习包 → **智谱账号级 QPM 限流 429** → 前端秒返"AI 服务暂时不可用"。**根因**：`packGenerator.callGlm` 只做 non-ok 抛异常，无 429 重试。**修复**：加 429 指数退避重试 3 次（10s → 30s → 60s），生产已 scp + restart。**教训**：任何外部 AI API 调用**必须实现 rate-limit-aware retry**（429 / 1305 / 1113 都触发退避），不能第一发失败就抛给用户。上升规则候选：CLAUDE.md §Guardrails "No Shortcuts" 加"AI/外部服务调用必须实现指数退避重试（至少 429/503 → 3 次 exponential backoff）"。
- `[pending]` **失败页按钮语义混淆**：Import 失败页两个按钮"回首页重试"和"试试别的"实际都是 `router.replace('/')` + 存一个 url 到 AsyncStorage 让 PasteBar 预填 —— Sprint 10 v10 后 PasteBar 已删除，第一个按钮实际啥也没做。**修复**：按钮 1 改成真正的原地重试（重发 /api/episodes/import-url + 用新 jobId 替换 route），按钮 2 保持"回首页"。**教训**：删除组件（PasteBar）时必须搜引用是否有隐式依赖（如通过 AsyncStorage 传值），否则老逻辑的入口消失但代码还在跑。

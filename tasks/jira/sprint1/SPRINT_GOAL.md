# Sprint 1 Goal（CR-001 应用后重写）

**Sprint**: 1（Spike Sprint）
**Duration**: 计划 4-6 天
**Goal**: 6 个 Spike 全部 VIABLE + Expo App 骨架跑通 + Backend Docker 上线 + Playwright web 测试链路验证

---

## Success Metrics

- SPIKE-001~006 全部产出 `docs/spike-results/SPIKE-NNN.md`，结论 VIABLE
- `bash scripts/start-web.sh` 起 `expo --web` 后 Playwright 可访问首页
- `bash scripts/start-backend.sh` 后 `curl http://localhost:3001/health` 返回 200 < 100ms
- 生产 backend `https://api.k0.yiiling.cn/health` 部署完成并返回 200
- iOS App 首页 3 入口在 iPhone 15 Pro Simulator（如可用）和 react-native-web 上都渲染正确
...
| SPIKE-005 | Spike | react-native-web + Playwright 集成可行性 |
| SPIKE-006 | Spike | Expo EAS Build 从 Windows 出 iOS TestFlight 包 |
